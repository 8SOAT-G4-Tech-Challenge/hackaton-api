...
export class NotificationService {
    ...
    async getNotifications(): Promise<Notification[]> {
        ...
        const notifications: Notification[] = await this.notificationRepository.getNotifications();
        return notifications;
    }

    async getNotificationById(id: string): Promise<Notification | null> {
        logger.info(`[NOTIFICATION SERVICE] Listing file by ID: ${id}`);
        const notification: Notification | null = await this.notificationRepository.getNotificationById(id);
        return notification;
    }

    async getNotificationsByUserId(userId: string): Promise<Notification[]> {
        logger.info(`[NOTIFICATION SERVICE] Listing files by user ID: ${userId}`);
        const notifications: Notification[] = await this.notificationRepository.getNotificationsByUserId(userId);
        return notifications;
    }
}
