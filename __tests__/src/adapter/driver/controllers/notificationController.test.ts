...
describe('NotificationController', () => {
    ...
    describe('getNotificationById', () => {
        it('should return the notification with status 200 when found', async () => {
            const notification = new NotificationMockBuilder().withId('notification-1').build();
            ...
            expect(notificationService.getNotificationById).toHaveBeenCalledWith('notification-1');
        ...
        it('should return 404 when the notification is not found', async () => {
            ...
            expect(notificationService.getNotificationById).toHaveBeenCalledWith('non-existent-id');
    ...
    describe('getNotificationsByUserId', () => {
        it("should return the user's notifications list with status 200", async () => {
            const notification1 = new NotificationMockBuilder().withId('notif-1').build();
            const notification2 = new NotificationMockBuilder().withId('notif-2').build();
            notificationService.getNotificationsByUserId.mockResolvedValue([notification1, notification2]);

            fakeReq = { user: { id: 'user-123' } };

            await notificationController.getNotificationsByUserId(fakeReq, fakeReply);

            expect(notificationService.getNotificationsByUserId).toHaveBeenCalledWith('user-123');
            expect(fakeReply.code).toHaveBeenCalledWith(StatusCodes.OK);
            expect(fakeReply.send).toHaveBeenCalledWith([notification1, notification2]);
        });
    });
});
