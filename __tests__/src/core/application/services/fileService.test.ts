...
describe('FileService', () => {
    ...
    describe('createFile', () => {
        ...
        it('should create the file and return the created file with status "initialized"', async () => {
            ...
            const result = await fileService.createFile(validCreateParams, validVideoFile);
    ...
    describe('updateFile', () => {
        ...
        it('should update the file and trigger a notification', async () => {
            const existingFile = new FileMockBuilder().withId('file-1').withUserId('user-1').build();
            const updatedFile = new FileMockBuilder().withId('file-1').withUserId('user-1').build();
            ...
            expect(fileRepository.getFileByIdOrThrow).toHaveBeenCalledWith(validUpdateParams.id);
    ...
    describe('getSignedUrl', () => {
        it('should return the signed URL for the given file', async () => {
            const file = new FileMockBuilder().withId('file-1').withUserId('user-1').build();
    ...
    describe('deleteFile', () => {
        ...
        it('should throw an error if the file is not found', async () => {
            ...
            await expect(fileService.deleteFile('non-existent-file')).rejects.toThrow(InvalidFileException);
        });
    });
});
